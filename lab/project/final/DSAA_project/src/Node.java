...
public class Node implements Comparable{
    ...
    public void findNext(PriorityQueue<Node> open, Map<int[][], String> openJudge, Map<int[][], String> close){
        Node currentState = this;
        close.put(currentState.board ,arrayToString(currentState));
        ...
        for (int i = 0; i < currentState.x0.length; i++) {
            ...
            if (!close.containsValue(arrayToString(next1))){
                ...
                if(flagofblock1==false&&flagofblock2==false&&flagofblock3==false){
                    //属于1*1，直接打印就行
                    next1.movement = currentState.board[x - 1][y] + " D";
                    if (openJudge.containsValue(n1)) {
                        open.offer(next1);
                    }
                    else {
                        open.offer(next1);
                        openJudge.put(next1.board, n1);
                    }
                ...
                if(flagofblock1==true&&flagofblock2==false&&flagofblock3==false){
                    ...
                    if(j==1){//代表就是左上角的值
                        next1.movement = currentState.board[x - 1][y] + " D";
                        if (openJudge.containsValue(arrayToString(next1))) {
                            open.offer(next1);
                        }
                        else {
                            open.offer(next1);
                            openJudge.put(next1.board, arrayToString(next1));
                        }
                    }
                    else{//在它左边的数字就是左上角的值
            ...
            if (!close.containsValue(n3)){
                ...
                if(flagofblock1==false&&flagofblock2==false&&flagofblock3==false){
                    //属于1*1，直接打印就行
                    next3.movement = currentState.board[x][y - 1] + " R";
                    if (openJudge.containsValue(n3)) {
                        open.offer(next3);
                    }
                    else {
                        open.offer(next3);
                        openJudge.put(next3.board, n3);
                    }
                }
            ...
            if (!close.containsValue(n4)){
                ...
                if(flagofblock1==false&&flagofblock2==false&&flagofblock3==false){
                    //属于1*1，直接打印就行
                    next4.movement = currentState.board[x][y + 1] + " L";
                    if (openJudge.containsValue(n4)) {
                        open.offer(next4);
                    }
                    else {
                        open.offer(next4);
                        openJudge.put(next4.board, n4);
                    }
    ...
    public static Stack<int[][]> answer(Node initState){
        ...
        java.util.Stack<String> move = new java.util.Stack<String>();
        java.util.Stack<String> boards = new java.util.Stack<String>();
        java.util.Stack<int[][]> output = new java.util.Stack<>();

        //将初始状态放入
        open.offer(initState);
        openJudge.put(initState.board, arrayToString(initState));
